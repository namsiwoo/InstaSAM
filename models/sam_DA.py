...
class SAM(nn.Module):
    ...
    def make_discriminator(self, type):
        self.type = type
        if type == 1:
            self.c_dim = 256
            self.DA_num_heads = 8
    ...
    def optimize_parameters(self, point_prompt=None, img_name=None, semi=False, epoch=0):
        ...
        del self.loss_G, self.pred_mask2, self.masks_hq2, self.space_query, self.channel_query
        # train discriminator....
        ...
        del self.input1, self.input2, self.loss_dis, self.loss_dis2, self.loss_dis3, self.pred_mask2, self.masks_hq2
