...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        if mask_token_only == True:
            if local_path == True:
                masks, iou_preds = self.predict_masks_local(
            ...
            for i_batch in range(len(image_embeddings)):
                mask, iou_pred = self.predict_masks_hq(
    ...
    #     iou_preds = []
    #
    #     if len(image_embeddings) == len(sparse_prompt_embeddings):
    ...
    #             iou_preds.append(iou_pred)
    #     else:
    #         # for i_batch in range(len(sparse_prompt_embeddings)):
    #         mask, iou_pred = self.predict_masks(
    #             image_embeddings=image_embeddings.unsqueeze(0),
    #             image_pe=image_pe,
    #             sparse_prompt_embeddings=sparse_prompt_embeddings,
    #             dense_prompt_embeddings=dense_prompt_embeddings,
    #             hq_feature=image_embeddings.unsqueeze(0)#hq_features
    #         )
    #         masks.append(mask)
    #         iou_preds.append(iou_pred)
    #     masks = torch.cat(masks, 0)
    ...
    #         iou_preds = iou_preds[:, mask_slice]
    #
    #     masks_hq = masks[:, slice(self.num_mask_tokens, self.num_mask_tokens+self.num_hq_token), :, :] # self.num_mask_tokens - 1, self.num_mask_tokens + 1
    #     # print(slice(self.num_mask_tokens-1, self.num_mask_tokens+1))
    ...
    #     hs, src = self.transformer(src, pos_src, tokens)
    #     iou_token_out = hs[:, 0, :]
    #     mask_tokens_out = hs[:, 1: (1 + self.num_mask_tokens + self.num_hq_token), :] # +1 for hq 2
    #
    #     # Upscale mask embeddings and predict masks using the mask tokens
    #     src = src.transpose(1, 2).view(b, c, h, w)
    #
    ...
    #     # upscaled_embedding_ours = self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature
    #     upscaled_embedding_ours = upscaled_embedding_sam
    #
    #     hyper_in_list: List[torch.Tensor] = []
    #     for i in range(self.num_mask_tokens+self.num_hq_token):  # +1 edit for hq 2
    #         if i < 4:
    #             # print(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]).shape, '--')
    #             hyper_in_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]))
    #         else:
    #             # print(mask_tokens_out[:, i:, :].shape)  #1, 1, 256
    #             # print(self.hf_mlp(mask_tokens_out[0, i:, :]).shape, '-') # 1, 32
    #             # print(upscaled_embedding_ours.shape) # 1, 32, 256, 256
    #             hyper_in_list.append(self.hf_mlp[i-4](mask_tokens_out[:, i, :]))
    #
    #     hyper_in = torch.stack(hyper_in_list, dim=1)
    #     # print(self.num_mask_tokens)
    #     # print(hyper_in.shape, '-0000') # 1, 6, 32
