...
from datasets.MoNuSeg_dataset import DA_dataset
...
def assign_data_path(data_name):
    if data_name == "MoNuSeg":
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/MoNuSeg/MoNuSeg/via instance learning data_for_train/MoNuSeg'
        else:
            data_name = '/media/NAS/nas_70/open_dataset/MoNuSeg/MoNuSeg_shift{:s}/via instance learning data_for_train/MoNuSeg_shift{:s}'.format(args.shift)
    elif data_name == "CPM":
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/CPM/CPM 17/via instance learning data_for_train/CPM 17'
        else:
            data_name = '/media/NAS/nas_70/open_dataset/CPM/CPM 17_shift{:s}/via instance learning data_for_train/CPM 17_shift{:s}'.format(args.shift)
    elif data_name == "CoNSeP":
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/CoNSeP/CoNSeP/via instance learning data_for_train/CoNSeP'
    elif data_name == "TNBC":
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/TNBC/TNBC_new/via instance learning data_for_train/TNBC_new'
        else:
            data_name = 'not yet'
    elif data_name == "segpc":
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/segpc/segpc'
        else:
            data_name = 'not yet'
    elif data_name == 'pannuke':
        if args.shift == 0:
            data_name = '/media/NAS/nas_70/open_dataset/pannuke/Pannuke_patch'
    elif data_name == 'cellpose':
        data_name = '/media/NAS/nas_70/open_dataset/Cellpose'
    elif data_name == 'DeepCell':
        data_name = '/media/NAS/nas_70/open_dataset/DeepCell'
    elif data_name == 'galaxy':
        data_name = '/media/NAS/nas_187/datasets/galaxy_dataset_UNIST'
    elif data_name == 'DeepLIIF':
        data_name = '/media/NAS/nas_70/open_dataset/DeepLIIF/DeepLIIF'
    elif data_name == 'DeepLIIF_BC':
        data_name = '/media/NAS/nas_70/open_dataset/DeepLIIF/BC'
    else:
...
def main(args):
    ...
    if args.dis_type == 1:
        ...
        sam_model.optimizer_dis = torch.optim.AdamW(sam_model.discriminator.parameters(), lr=args.lr*0.5)
        sam_model.optimizer_dis2 = torch.optim.AdamW(sam_model.netD_mask.parameters(), lr=args.lr*0.5)
        sam_model.optimizer_dis3 = torch.optim.AdamW(sam_model.netD_offset.parameters(), lr=args.lr*0.5)
    ...
    train_dataset = DA_dataset(args, 'train', use_mask=args.sup, data=(args.data1, args.data2), train_IHC=args.train_IHC)
    val_dataset = DA_dataset(args, 'val', use_mask=args.sup, data=(args.data1, args.data2), train_IHC=args.train_IHC)
    ...
    #     sam_model = load_checkpoint(sam_model, os.path.join(args.model, str(epoch)+ '_model.pth'))
        ...
        for iter, batch in enumerate(train_dataloader): # batch[0]
            ...
            loss = space_loss1 +space_loss2+ channel_loss1+channel_loss2# + bce_loss + iou_loss + offset_loss + bce_local_loss + iou_local_loss

            train_loss += loss / len(train_dataloader)

            if (iter + 1) % args.print_fq == 0:
                print('{}/{} epoch, {}/{} batch, train loss: {}, bce: {}, iou: {}, offset: {} // dis_binary: {} dis_offset: {} // space: {}, channel: {}'.format(epoch,
                                                                                                        args.epochs,
                                                                                                        iter + 1,
                                                                                                        len(train_dataloader),
                                                                                                        loss, bce_loss,
                                                                                                        iou_loss,
                                                                                                        offset_loss, space_loss1,
                                                                                                        channel_loss1, space_loss2, channel_loss2))
