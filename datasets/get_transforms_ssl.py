...
class LabelEncoding(object):
    ...
    def __call__(self, imgs):
        ...
        out_imgs = list(imgs[:-self.num_labels-self.num_points]) # append only image

        for i in range(-self.num_points-self.num_labels, -self.num_labels):
            label = imgs[i]
            ...
            for i, region in enumerate(label_regions):
                ...
                point[round(region.centroid[0]), round(region.centroid[1])] = i+1

            # point_labels = np.ones(len(point_coords), dtype=int)
            # point_coords = np.array(point_coords)
            point = Image.fromarray(point.astype(np.uint16))
...
selector = {
    ...
    'label_encoding': lambda x: LabelEncoding(x[0], x[1]),  # take effect on RGB labels
