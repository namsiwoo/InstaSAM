...
class DA_dataset(torch.utils.data.Dataset): #MO, CPM, CoNSeP
    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            img2 = Image.open(os.path.join(self.data2, self.path2, self.split, img_name)).convert('L') #'RGB'

            img_name = self.samples[0][index % len(self.samples[0])]
            img1 = Image.open(os.path.join(self.data1, self.path1, self.split, img_name)).convert('L') #'RGB'
